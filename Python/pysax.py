...
###############################################################
###                                                         ###
###     Code taken from https://github.com/dolaameng/pysax  ###
###             All credit goes to dolaameng                ###
###                                                         ###
###############################################################
###############################################################
...
class SAXModel(object):
	def __init__(self, window = None, stride = None, 
		...
		vecs = list(map((lambda obj: (obj[0]+obj[1])/2), zip(cps, cps[1:])))#zip(cps, cps[1:])) ## taking mean may not be accurate
		vecs[0] = cps[1]
		vecs[-1] = cps[-2]
